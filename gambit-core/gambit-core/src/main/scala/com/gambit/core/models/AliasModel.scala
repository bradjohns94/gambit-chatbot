...
class AliasReference(db: Database) {
   ...
   */
  def getPrimaryName(name: String): Future[String] = db.run(getPrimaryNameAction(name))
   ...
   *  @return the mapped alias name if it exists, otherwise the provided name
   */
  def getPrimaryNameAction(name: String): DBIO[String] = {
    aliasTable.filter{_.aliasedName === name}.map{_.primaryName}.result.map{
      _.headOption.getOrElse(name)
    }
  }
}
